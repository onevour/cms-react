...
class EmployeeAngkaKreditForm extends Component {
    ...
    submitForm(e) {
        ...
        this.props.updateCredit(param)
        this.setState({request: 1})
    }

    handleRemove(o, index) {
        swal({
            title: "Hapus Angka Kredit",
            text: "hapus Angka Kredit tahun " + formatYear(o.tahun) + " nilai : " + o.nilai_rata + "?",
            icon: "info",
            buttons: true,
            dangerMode: false,
        }).then((willDelete) => {
            if (willDelete) {
                this.props.removeCredit(o)
            }
        });
    }
